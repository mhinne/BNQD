...
#os.environ['PROJ_LIB'] = r'C:\\Users\\Max\\Anaconda3\\pkgs\\proj4-5.2.0-ha925a31_1\\Library\\share'

#import mpl_toolkits
#mpl_toolkits.__path__.append('C:\\Users\\Max\\Anaconda3\\envs\\gprdd36\\Lib\\site-packages\\mpl_toolkits')
...
import importlib
import BNPQED
importlib.reload(BNPQED)

print("GPy version:      {}".format(GPy.__version__))
print("GPRDD version:    {}".format(BNPQED.__version__))
...
def read_data(votesfile='datasets\\Dutch parliament elections 2017\\Gemeente_uitslagen_TK2017.csv', 
              geofile='datasets\\Dutch parliament elections 2017\\Gemeenten_2019_locations.csv'): 
    print('Reading data')
    ...
    with open(geofile, 'r') as csvfile:
        ...
        for row in reader:
            geo_data[row['NAAM']] = {'Provincie': row['Provincie'], 'x': float(row['Lon'].replace(',', '.')), 'y': float(row['Lat'].replace(',', '.'))}
...
def read_province_data(provincefile='datasets\\Dutch parliament elections 2017\\provinciegrenzen\\Provincies Nederand KML _ LocalFocus.xlsx'):
    ...
    for i, row in enumerate(ws_source.iter_rows(min_row=startsAt, max_row=13)):        
        ...
        province_surface = ws_source.cell(row = i+startsAt, column = province_polygon_col).value
...
def read_bible_belt_data(bbfile='datasets\\Dutch parliament elections 2017\\biblebelt.txt'):
    with open(bbfile, 'r') as file:
        biblebelt = list(set(file.read().split(', ')))
    return biblebelt
...
def flat_plot(x, y, z, ix, cmap=cm.viridis, marker_alpha=0.7, label1='Set 1', label2='Set 2', title=None):
    
    fig = plt.figure(figsize=(9,9))
    m = Basemap(projection='merc', llcrnrlat=NL_ll_lat, urcrnrlat=NL_ur_lat, llcrnrlon=NL_ll_lon, urcrnrlon=NL_ur_lon, resolution=map_resolution)
    m.drawmapboundary(fill_color='#d8eaed')
    m.drawcountries(linewidth=1.0)
    m.drawcoastlines()
    m.fillcontinents(color='#f0efe9', lake_color='#d8eaed')
    m.scatter(x[ix], y[ix], 
              c=z[ix], alpha=marker_alpha, cmap=cmap, marker='s', zorder=10, latlon=True, label=label1, edgecolor='k')
    sc = m.scatter(x[np.logical_not(ix)], y[np.logical_not(ix)], 
              s=30, c=z[np.logical_not(ix)], alpha=marker_alpha, cmap=cmap, marker='o', zorder=10, latlon=True, label=label2, edgecolor='k')
    if title:
        plt.title(title)
...
#    ax = fig.gca(projection='3d')
    ...
    m.scatter(x[ix], y[ix], z[ix],
              c=z[ix], alpha=marker_alpha, cmap=cmap, marker='s', zorder=10, latlon=True, label=label1, edgecolor='k')
    m.scatter(x[np.logical_not(ix)], y[np.logical_not(ix)], z[np.logical_not(ix)],
              s=30, c=z[np.logical_not(ix)], alpha=marker_alpha, cmap=cmap, marker='o', zorder=10, latlon=True, label=label2, edgecolor='k')
    if title:
        plt.title(title)
    plt.legend(loc='best')
...
def flat_draw_border(ax, m, a, b, xmin=NL_ll_lon, xmax=NL_ur_lon, label='Line'):
    ...
    xx, yy = m(x, y)
    ax.plot(xx, yy, linestyle='--', linewidth=3.0, label=label, color='k', zorder=10)
...
def full_draw_border(fig, m, a, b, xmin=NL_ll_lon, xmax=NL_ur_lon, label='Line'):
    ...
    ax = fig.gca()
    xx, yy = m(x, y)
#    line = art3d.Line3D(xx, yy, 1e-5*np.ones((100)))
...
def full_province(fig, m, province):
    ...
    pplc = PolyCollection(province_polys, edgecolor='black', facecolor='#ffffff', closed=False)    
...
y = np.array([float(data[label]['PVV'] + data[label]['SP'] + data[label]['50PLUS'] + data[label]['FvD'] ) / total_votes[label] for label in data.keys()])
...
Total = BNPQED.BnpQedAnalysis(X, y, kernel_dict, labelFunc, b=border_pred, opts=opts)
Total.train()
#    total_log_bf = Total.get_total_log_Bayes_factor()
...
fig, axes = plt.subplots(nrows=1, ncols=len(kernels), figsize=(18,4), sharex=True, sharey=True)    
for i, (k, v) in zip(range(len(kernels)), kernel_dict.items()): 
    ...
    axes[i].plot(border_x1, es_mean, label='Effect size', linewidth=1.0, linestyle='--', color=color_disc)
    axes[i].fill_between(border_x1, np.squeeze(es_mean + 0.5*es_std), np.squeeze(es_mean - 0.5*es_std), 
        color=color_disc, alpha=0.3)
    axes[i].annotate('log BF = {:0.1f}'.format(preds['logbayesfactor']), (3.8, 0.12))
...
for k in ['Linear', 'Exponential']:
    ...
    if printFig:
        dpi = 250
        plt.savefig('figures\\Dutch elections\\Dutch2017_elections_{:s}_kernel_{:d}dpi_c.png'.format(k, dpi), format='png', dpi=dpi)
