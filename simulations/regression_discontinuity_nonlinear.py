...
def random_predictors(n, xmin=-3, xmax=3):
    ...
    #
#
def periodic_data_disc(n=50, bias=1.2, period=6, b=0.0, amp=2.0, disc=-1.0, noise_sd=1.0):
    x = get_predictors(n)    
    f = bias + amp*np.sin((2*np.pi/period)*(x)) + disc*(x>b)
...
bma_bfs = np.zeros((num_sims, M))
pvals = np.zeros((num_sims, M))
...
plt.suptitle('Nonlinear true process')
