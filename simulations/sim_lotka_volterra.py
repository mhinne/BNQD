...
Y = lotka_volterra.simulate_rk45(init_pos=init_pos, x_start=x_start, x_end=x_end, h=h)
# X, Y = lotka_volterra.get_random_samples(Y=Y, N=200, sigma_x=1, x_start=x_start, x_end=x_end)
n_samples = int((x_end-x_start)/h)
X_temp = np.zeros((n_samples, 2))
for i in range(2):
    X_temp[:,i] = np.arange(x_start, x_end, step=h)

fig, ax = lotka_volterra.plot_timeseries(X_temp, Y, x0)
fig.suptitle(f"Lotka volterra with alpha discontinuity: {alpha} to {alpha_disc} at $x_0$={x0}")
plt.show()

## Run BND design
sm = SpectralMixture(Q=6, x=X_temp[:,0], y=Y[:,0])
