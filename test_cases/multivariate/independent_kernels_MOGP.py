...
from gpflow.kernels import Matern32, Linear
from kernels import MultiOutputKernel, IndependentKernel
...
plt.figure()
plt.plot(x_1, y_1, 'o', label='$x_1$')
plt.plot(x_2, y_2, 'o', label='$x_2$')
plt.legend()
plt.show()
...
rank = 2  # Rank of mixing matrix W

base_kernel = Matern32(active_dims=[0])
# base_kernel = Linear(active_dims=[0])

independent_kernel = IndependentKernel([base_kernel, base_kernel], x0=0.5, forcing_variable=0)
mogp_kernel = MultiOutputKernel(base_kernel=independent_kernel, output_dim=p, rank=rank)
